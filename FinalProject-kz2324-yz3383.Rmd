...
In this project, we explore the relationship of several variables from Yelp API including price, cuisine, rating, number of reviews and coordinates. We also joined the dataset with borough information for further exploration. 
...
Here is the [Python code](https://github.com/zhangyue9966/Final_Project_STAT5293/blob/master/scrape_yelp/scrape_yelp_western.py)  for getting western restaurants information.
...
All the records are independent and most of the data are categorical. Moreover, the missing value is not extensive compared with the whole dataset. Thus, it is not meaningful to use methods like listwise, pairwise or regression to fill up missing value. In this project, records contaning missing value are deleted directly.
...
### 1. Is the distribution of restaurant in different cuisine related to borough?
...
In this project, we performs explorative data analysis on data generated from Yelp API. We explored the relationship among borough, rating, cuisine, price and number of reviews.

#### Way to improve

Due to our limited access to the Yelp data, most of our variables are discrete and restrict our ways to explore the data. In this project, most of our plots are barcharts or mosaic plots as we have many discrete variables. 
If we still have time we will seek more information and get more continuous data like date in order to analyze the change of customers preference with different generation. We are also interested in learning the reviews on Yelp, using wordcloud and n-gram clustering we can find more components which affect the rating of the restaurants.
