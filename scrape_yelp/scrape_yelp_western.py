    ...
    for term in terms:
        ...
        for offset in range(0, 1000, 50):
            ...
            if response.status_code == 200:
                ...
                business_data_western.extend(results)   
            elif response.status_code == 400:
                print('400 Bad Request')
                break
    return business_data_western

def output_total_data(df):
    df.to_csv('business_data_western.csv', index=False)
    data = df.to_json(orient='records')
    with open('business_data_western.json', 'w') as outfile:
        json.dump(json.loads(data), outfile, indent=4)

def main():
    categories = ['italian', 'spanish', 'greek','french', 'mexican']
    # Added 'food' to query options 
    terms = ['restaurants', 'food']
    queries = ['%s %s' % (cat, term) for term in terms for cat in categories]
    business_data_western = search_yelp(queries)

    df = pd.DataFrame(business_data_western)
